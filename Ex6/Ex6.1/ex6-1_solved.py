...
while True:
    ...
    if(len(var_types) != m):
        print(f"Precisa ter exatamente {m} variáveis, dos tipos \'Z\', \'R\' ou \'B\'")
    else:
        break
...
while True:
    vals: list[float] = list[float](input().split())   # Pega a entrada no formato (i j k) e converte pra uma lista
